...
import se.omegapoint.academy.opmarketplace.apigateway.infrastructure.json_representations.events.incoming.item.ItemPurchaseNotCompletedDTO;
...
public class ItemPurchaseListener implements Consumer<reactor.bus.Event<Event>> {
    ...
    public void accept(reactor.bus.Event<Event> event) {
        ...
        } else if (model instanceof ItemPurchaseNotCompletedDTO) {
            result.setErrorResult(ResponseEntity.badRequest().body(((ItemPurchaseNotCompletedDTO) model)));
        } else {
