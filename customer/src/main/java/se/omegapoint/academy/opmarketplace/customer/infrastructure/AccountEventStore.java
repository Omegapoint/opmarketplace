...
public class AccountEventStore implements Consumer<Event<DomainEvent>> {
    ...
    public void accept(Event<DomainEvent> event) {
        ...
        if(domainEvent.aggregateName().equals(Account.class.getSimpleName())){
            if (domainEvent.eventData().equals(AccountCreated.class.getSimpleName()) && accountInExistence(domainEvent.aggregateId()))
    ...
    public boolean accountInExistence(String email) {
        return repository.findByAggregateIdOrderByTime(email).size() > 0;
    }
}
