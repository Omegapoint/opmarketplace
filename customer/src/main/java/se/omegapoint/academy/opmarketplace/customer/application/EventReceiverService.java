    ...
    public ResponseEntity<Void> eventInput(@RequestBody RemoteEvent event) {
        ...
        try {
            if (event.getType().equals(AccountCreated.NAME))
                eventBus.notify(Selectors.object(AccountCreated.CHANNEL), Event.wrap(objectMapper.readValue(event.getData(), AccountCreatedJsonModel.class).domainEvent()));
            if (event.getType().equals(AccountUserChanged.NAME))
                eventBus.notify(Selectors.object(AccountUserChanged.CHANNEL), Event.wrap(objectMapper.readValue(event.getData(), AccountUserChangedJsonModel.class).domainEvent()));
